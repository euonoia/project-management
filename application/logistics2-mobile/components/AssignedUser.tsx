...
export default function AssignedUser({ navigation }: any) {
  ...
  useEffect(() => {
    ...
    interval = setInterval(fetchAssignedCustomers, 5000); // refresh every 5 seconds
  ...
  return (
    <View style={{ flex: 1, padding: 16 }}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={{ padding: 12, borderBottomWidth: 1, borderColor: "#eee" }}>
            <Text>
              Customer: {item.customer_name} ({item.customer_firstname} {item.customer_lastname})
            </Text>
            <Text>Pickup: {item.pickup_location}</Text>
            <Text>Dropoff: {item.dropoff_location}</Text>
            <Text>
              Vehicle: {item.vehicle_plate} ({item.car_brand} {item.model})
            </Text>
      ...
      <Button title="View Travel History" onPress={openTravelHistory} />
